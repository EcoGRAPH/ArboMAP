...
date: "Updated `r format(Sys.time(), '%B %d, %Y')` for Version 4.3"
output: 
...
The workflow for running a normal weekly report after set-up is divided into three main tasks (Figure \@ref(fig:userflow)). 

1. Acquire updated WNV surveillance and entomological data from state databases. 
2. Use the GEE app to obtain environmental data. 
3. Use the R project GUI to generate a report. 

```{r userflow, echo=FALSE, fig.align='center', out.width='100%', fig.cap="User-centered diagram showing the workflow steps and high-level code processes."}
knitr::include_graphics(file.path("figures", "arbomap_user_system.jpg"))
```

The system diagram on the right (Figure \@ref(fig:userflow)) shows the high-level processes for modeling and forecasting in the code. Raw mosquito WNV test results are modeled to create an infection rate statistic. Distributed lags handle the delayed effects of environmental conditions on disease dynamics. 
...
Other important modeling choices include determining which meteorological variables to use as predictors, and which type of mosquito index to use. Based on previous research, the best results seem to be obtained by combining temperature with a variable related to moisture (e.g., vapor pressure deficit, relative humidity, or precipitation). 

It is also necessary to choose a technique for modeling the mosquito infection data to generate entomological risk indices. In most situations, the mosquito infection growth rate (MIGR) is the best model of entomological risk, particularly in the early part of the WNV season. In situations where the mosquito infection rates does not exhibit unimodal growth in the early season, the mosquito infection intercept (MII) or the area under the mosquito infection growth curve (AUC) can be used as alternatives. For MIGR and MII, there is the also the option of implementing a spatially stratified version where multiple can be use in different parts of the state. There is also a very simple percent positive mosquito pool option. See the [Parameters](#params) section for details on how to pick the `mosquito_model`. 

As there may be more years of historical human data available than mosquito pool data, for unknown years, ArboMAP will impute for the missing mosquito infection rate (MIR) statistic. The imputation is based on a linear relationship between the known MIR values (calculated from the model above) and the total human cases for that year. This imputation methods helps preserve the relationship of the MIR in forecasting without becoming overly influenced by the environmental factors. In the case there is no mosquito data available yet for the forecast year (when there is no human data yet), an average risk of 0 is imputed (the MIR statistic is centered before being used in forecasting).  
...
Optional: Only if a stratified mosquito model (`mosquito_model = stratifiedMIGR` or `stratifiedMII`) is used, then a spatial stratification file must be provided. An example stratification file for South Dakota is provided with the demonstration dataset. 
...
* `mosquito_model` is a code for the type of mosquito infection rate model to be used: 
  ...
  * `MIGR`: Mosquito infection growth rate (the default if text cannot be matched to another option)
...
Due to improvements in memory handling, it is recommended to have at version 4.2.1 or later of base R, especially if the computer has limited RAM. 
...
When preparing to start running weekly reports for the new WNV season, there are three main steps that are needed annually: 1) acquiring data, 2) updating parameters, and 3) updating software, if necessary. 
...
The last mosquito graph shows the relative risk due to the mosquito infection rate as a time-series of all modeled years.
...
* Wimberly, M. C., J. K. Davis, M. B Hildreth, and J. L. Clayton. 2022. Integrated Forecasts Based on Public Health Surveillance and Meteorological Data Predict West Nile Virus in a High-Risk Region of North America. Environmental Health Perspectives 130 (8) August 2022. doi: https://doi.org/10.1289/EHP10287 
