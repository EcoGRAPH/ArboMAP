...
ArboMAP uses three main sources of data: human case, mosquito pool, and weather data. Each data format is described in the following sections, along with a detailed explanation of the report parameters. Each data source has its own folder within the ArboMAP project (Figure \ref{fig:data_folders}).

```{r data_folders, echo=FALSE, fig.align='center', out.height='25%', fig.cap="\\label{fig:data_folders}The data folders within the ArboMAP project, highlighted inside of the red box."}
...
* `year_human_end`: End year of human data to use. Unless you are running some kind of special tests or reports, this should be the year before the current/forecast year. See the "Human case data" section under "ArboMAP data requirements & parameters" for a discussion on this.
...
1) Click on the "Run" button in the top-middle pane (Step 1 in Figure \ref{fig:gee_code}). This will bring up the user interface in a panel on the left-side. 
2) Under the "Downloader" section, select the state from the drop down list (first part of Step 2 in Figure \ref{fig:gee_code}). 
3) Change the download start and end dates as necessary (second part of Step 2 in Figure \ref{fig:gee_code}). As gridMET data are updated from the early preliminary data that are initially released, it is a good idea to include 'extra' recent history in the weekly updates. The default end date is the date of latest available data, and the default start date is a month prior. 
4) In the "Task" pane, click on the blue button labeled "RUN" to kick off the processing (Step 3 in Figure \ref{fig:gee_code}). The file will be saved to the user's Google drive when finished.  


```{r gee_code, echo=FALSE, fig.align='center', out.width='80%', fig.cap="\\label{fig:gee_code}Code Editor interface for the ArboMAP Google Earth Engine script to download gridMET weather data. The primary steps are labeled with key items marked."}
...
2) Under the "Downloader" section, select the state from the drop down list (Step 1 in Figure \ref{fig:gee_app})
3) Change the download start and end dates as necessary (second part of Step 2 in Figure \ref{fig:gee_code}). As gridMET data are updated from the early preliminary data that are initially released, it is a good idea to include 'extra' recent history in the weekly updates. The default end date is the date of latest available data, and the default start date is a month prior.
4) In the small window that pops-up on the map, click the download link, which will download the file directly from the browser to your computer (Step 3 in Figure \ref{fig:gee_app}).

```{r gee_app, echo=FALSE, fig.align='center', out.width='80%', fig.cap="\\label{fig:gee_app}ArboMAP Google Earth Engine app to download gridMET weather data. The primary steps are labeled with key items marked."}
...
It is easiest to download R from the Comprehensive R Archive Network (CRAN): **<https://cloud.r-project.org>**. For Windows user, please click on the link for "Download R for Windows." Choose "base," then "Download R for Windows." Run this file and install R on your system. Use the default settings for the installer.
...
2) In RStudio, in the Files pane, click on `annual_midApril_tex_update.R` (Figure \ref{fig:annual_file}). 
```{r annual_file, echo=FALSE, fig.align='center', out.height='25%', fig.cap="\\label{fig:annual_file}RStudio Files pane showing the annual TeX update script circled."}
knitr::include_graphics(file.path("figures", "files_rstudio_annualtex.jpg"))
```
3) In the Source pane that opens up, make sure the cursor is at the start of line 1, and hit "Run" (Figure \ref{fig:annual_tex}). 
```{r annual_tex, echo=FALSE, fig.align='center', out.width='60%', fig.cap="\\label{fig:annual_tex}Annual TeX update script as seen in RStudio Source pane."}
