...
1. Annual TeX Live update (to create pdfs):
...
2. Update human data: At the beginning of a new season, i.e. a new forecast year, the previous year's human case data will need to be added to the single CSV file, in the `data_human` folder, with these required fields:
    * County ID field: This will either be the county FIPS code (`r fields_fips`) or the county name (`r fields_names`).
    * `date`: the onset date of the symptoms of the case (MM/DD/YYYY or in non-ambiguous formats such as YYYY-MM-DD) 
3. Update off-season weather data via GEE app: 
...
4. Annual parameter updates: The defaults can be updated by manually editing the header of the `ArboMAP_forecast.Rmd` file, or the correct setting or file can be picked from the interactive user interface each time. 
