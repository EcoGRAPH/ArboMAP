...
```{r forecast_summary, echo=FALSE, include=FALSE}

# Creating various summary tables, calculating model averages,
#   setting up common theme sets for use in multiple places for display
...
# average predictions for current year
preds_ave_fc_yr <- preds_ave %>% 
  dplyr::filter(year_epi == epiyear_request)

#and for just this forecast week
preds_ave_fc_wk <- preds_ave_fc_yr %>% 
  dplyr::filter(week_epi == epiweek_request) 


## Chart-related
# Also adding common map themes used in many places
theme_arbo_map <- theme(
  # panels
...
#fix common theme, take a look at poster ggplots 
# fix legend
# don't worry about color scheme yet
# lower: 0 - 12.5; ave 12.5 - 87.5; higher 87.5 - 100
# <<>> DEV

# <<>> DEV Add TABLE of any counties with higher than average risk
...
sf_abs_risk <- data_sf_orig %>% 
  dplyr::left_join(preds_ave_fc_wk, by = "arbo_ID")
...
#<<>> DEV make pretty, coordinate color schemes ish with p_rel_risk
...
# request for legend to include both 
# numerical risk/percentage/probabilities
# AND descriptives
...
# [V3] censored pred values displayed to be between 'modeled' period
#   betwen min/max weeks from alpha calculation.
# Removed to see what happens <<>> DEV

# DEV
# <<>> preds pre/post current week line type change: censored pred <<>><<>>
# <<>> censor weeks to .... human alpha range? (modeled) ... any obs-OR-pred >0? 

preds_ave_fc_yr_st <- preds_ave_fc_yr %>% 
  #collapse districts into state values 
  dplyr::group_by(week_epi) %>% 
  dplyr::summarise(pred_ave = mean(pred_ave, na.rm = TRUE),
                   pred_min = mean(pred_min, na.rm = TRUE),
                   pred_max = mean(pred_max, na.rm = TRUE))

p_cur_yr_fc <- ggplot2::ggplot() +
  #min/max ribbon first so that ave pred plots on top
  ggplot2::geom_ribbon(data = preds_ave_fc_yr_st,
                       aes(x = week_epi,
                           ymin = pred_min,
                           ymax = pred_max),
                       fill = 'grey80') + 
  ggplot2::geom_line(data = preds_ave_fc_yr_st, 
                      aes(x = week_epi,
                          y = pred_ave),
                     color = 'red') 
