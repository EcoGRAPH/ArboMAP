...
for (this_model in model_names){
  ...
  model_plots[[this_model]] <- plot(
    this_reg, select=1,
    # [DMN] adding shift with coef to make more interpretable
    shift = coef(this_reg)[1]
    ## [DMN] adding data to potentially plot
    #rug = TRUE,
    # [DMN] adding residuals
    #residuals = TRUE
    ## DEV TAKING WAY TOO LONG NOW. REMOVE???? <<>>
  ) 
...
The following map displays the **absolute risk** of predicted positive districts during epidemiological week `r epiweek_request`. 

This map can be used in conjuction with the **relative risk** map. The absolute risk map shows the risk of a county reporting at least one WNV positive human case during this week, and the relative risk map shows if this risk is elevated (or not) as compared to previous years. 
...
In the forecast week (Week `r epiweek_request`), there are `r rel_risk_distr_count` counties with higher than average rest as compared to the same epidemiological week in previous years (`r max(human_exists_yr_min, params$year_modeling_start)` through `r params$year_modeling_end`). 
...
```{r current_year_forecast, echo=FALSE, include=TRUE, fig.align='center', fig.width=6, fig.height=3}
...
After data processing, the human case data contained a total of `r dx_human_nrow_2clean` rows containing data from years: `r human_exists_yr_list`. Parameters were set to include human data from `r params$year_human_start` through `r params$year_human_end`, and there were `r length(human_yrs_missing)` years not found in the given data. `r if(length(human_yrs_missing)>0){paste0("The following years were not found in the dataset:", human_yrs_missing %>% sort() %>% stringr::str_c(collapse=", "), ".", " Confirm that these years were zero-case years, and not data missing from the dataset.")}` 

Over all years, the state saw a cumulative total of `r state_cases_total` human cases representing `r state_any_cases_total` positive district-weeks from a total of `r length(dx_human_districts)` counties.
...
After data processing, the mosquito pool data contained a total of `r dx_mosq_nrow_2clean` rows, containing data from years: `r human_exists_yr_list`. Overall during this timeframe, a total of `r length(dx_mosq_districts)` counties reported pool data. 

Parameters were set to include mosquito data from `r params$year_mosquito_start` through `r params$year_mosquito_end`, and there were `r length(mosq_yrs_missing)` years not found in the given data. `r if(length(mosq_yrs_missing)>0){paste0("The following years were not found in the dataset:", mosq_yrs_missing %>% sort() %>% stringr::str_c(collapse=", "), ".")}` Note that even if there were no positive pools in a given year, if there were any pools tested then the data will be useful; zero infection rates do predict low-risk years and should be used.

Modeling was requested from `r params$year_modeling_start` through `r params$year_modeling_end`. Years without mosquito data during these years are assumed to have average mosquito infection rates. This allows us to estimate relationships with environmental data even when mosquito data are not available. There were `r length(yrs_mir_imputed)` years where mosquito infection rates were imputed. `r if(length(yrs_mir_imputed)>0){paste0("These years are: ", yrs_mir_imputed %>% sort() %>% stringr::str_c(collapse=", "), ".")}` In modeling years where sufficient cmosquito data were present, the mosquito infection statistic was created using the model specified in the input parameter: `r mosquito_model_clean`. 

In the forecast year to date, there were `r dx_mosq_nrow_maxyr` pools reported from `r dx_mosq_districts_maxyr` counties. Of these pools `r mosq_pos_num_maxyr` (`r round_any(mosq_pos_perc_maxyr, 1)`%) were reported WNV positive. 
...
```{r mosquito_pools_2wks, echo=FALSE, include=TRUE, fig.align='center', out.width="75%"}
...
```{r mosquito_pools_posrate, out.width="100%", echo=FALSE, include=TRUE, fig.height=2.5}
...
Parameters were set to include weather data from `r params$year_weather_start` through `r params$year_weather_end`, and there were `r length(env_yrs_missing)` years not found in the given data. `r if(length(env_yrs_missing)>0){paste0("The following years were not found in the dataset:", env_yrs_missing %>% sort() %>% stringr::str_c(collapse=", "), ".")}`
...
# Attempt at handling single legend for both
# #difficult to get plots the same size if only showing one legend
# #pulling legend out, and graphing as if 3 plots
# get_legend <- function(my_gplot){
#   tmp <- ggplot_gtable(ggplot_build(my_gplot))
#   leg <- which(sapply(tmp$grobs, function(x) x$name) == "guide-box")
#   legend <- tmp$grobs[[leg]]
# }
# #get legend
# p_env_legend <- get_legend(p_env_var2_color) +
#   #strange background showed up that I haven't dealt with before
#   theme_void()
# #remove legend from env plot
# p_env_var2_color <- p_env_var2_color + 
...
cat("\\blandscape")
```
```{r cond_newpage_ref_map_start, echo=FALSE, results='asis', eval=(!sf_wider)}
...
`r unique(data_sf_orig$NAME) %>% sort() %>% stringr::str_c(collapse=", ")`

```{r reference_map, echo=FALSE, include=TRUE, fig.align='center', fig.width=if (sf_wider) 10 else 7.5}
