...
params:
  ...
  state_code: "SD"
  #predictors and mosquito settings
  predictor_v1: "tmeanc"
  predictor_v2: "vpd"
  mosquito_model: "AUC"
  ##data range settings
  ...
  human_file: !r file.path("data_human", "simulated human case data.csv")
  mosquito_file: !r file.path("data_mosquito", "simulated mosquito tests.csv")
  stratification_file: !r file.path("data_strata", "example SD strata.csv")
  #model_file: !r file.path("data_models", "models.txt")
  #TESTING<<>>
  model_file: !r file.path("dev", "models_tpfx.txt")
  weather_folder: "data_weather"
  #additional settings
...
# pull in models to run
model_in <- readr::read_csv(params$model_file, col_names = FALSE)
...
weatherlist <- list.files(path = params$weather_folder, pattern = "(.csv)", 
...
for (i in 1:length(weatherlist)) {
  ...
  if (weatherlist[[i]] != weathersummaryfile) {
    tempdf <- read_csv(file.path(weatherlist[[i]]))
...
if (resamplemosquitodata) {
  ...
  tempdf <- readr::read_csv(params$mosquito_file)
  
  # read and format the resampling file
  wnvresamplefile <- read_csv(params$resample_file)
  ...
  wnv <- readr::read_csv(params$mosquito_file)
...
if (mosqsummarystyle == "simpleratio") {
  ...
  randeffs <- wnv %>% 
    group_by(year) %>% 
    dplyr::summarise(totpos = sum(wnv_result, na.rm=TRUE),
                     tottest = n()) %>% 
    as.data.frame()
...
if (mosqsummarystyle %in% c("stratifiedMIGR", "stratifiedMII")) {
  ...
  # read stratum file
  strata <- readr::read_csv(params$stratification_file)
...
# import data
human <- readr::read_csv(params$human_file)
begrow <- nrow(human)
