...
params:
  ...
  mosquito_doy_start: 
    ...
    value: 140
  mosquito_doy_end: 
    label: "End of mosquito modeeling period (day of year)"
    value: 214
  ...
  file_models: 
    ...
    value: !r file.path("dev", "models_tpfx.txt") #!r file.path("data_models", "models.txt")
    input: file
  folder_weather: 
  ...
  year_weather_start: 
    ...
    value: 2000
  year_weather_end: 
    label: "End year of weather/environmentla data to use"
    value: 2018
  year_modeling_start: 
...
subtitle: "`r params$state_name`    \nArbovirus Modeling and Prediction (ArboMAP)"
...
# however, I believe they need to set that in YAML headers too, so really shouldn't be a problem here
...
data_sf_box <- data_sf_orig %>% 
  #get bounding box with xmin, xmax, ymin, ymax
  sf::st_bbox() %>% 
  #convert into tibble for calcs
  as.list() %>% 
  tibble::as_tibble_row() %>% 
  #calc x length and y length
  dplyr::mutate(x_len = abs(xmax - xmin),
                y_len = abs(ymax - ymin))
#TRUE if wider, to be used with dynamic sizing/page orientation #[DEV] 
sf_wider <- data_sf_box$x_len > data_sf_box$y_len
...
# If set up correctly, these should align, but it is not necessarily true
...
#number of districts in max year
dx_mosq_districts_maxyr <- data_mosq_maxyr %>% 
...
Rationale: WNV remains a persistent public health threat in many parts of the United States. Having advance warning on where and when WNV outbreaks might occur would allow for better targeting of public health responses. The transmission of mosquito-borne diseases, such as WNV, is influenced by environmental conditions that affect many aspects of the disease transmission system.  

Modelling: ArboMAP uses an ensemble of different mathematical models that each are predicting if a county will report at least one case in a given week ('positive district-week'). Results presented are an average of the models with ranges as appropriate. As part of the process, mosquito infection rate is also modeled based on the mosquito pool data, and is included in the default modelling. ArboMAP uses general additive models (GAMs) with smooths for seasonality, and also lagged weather data, which allows it to model the time-delayed effects of weather conditions. `r if(params$create_appendix){"The appendix will expand all the results to show each individual model."}`

The report will first present county-level relative and absolute predicted risk, time-series graphs for current and all year forecasts, estimated human case counts for the current year, and other statistics. The second division contains summaries and views into the input data. `r if(params$create_appendix){"The optional third section, an appendix, will include more detailed information about each of the models as well as advanced forecast model results."}`
...
## Forecast week WNV relative risk map

```{r relative_risk_map_calc, echo=FALSE, include=TRUE, fig.align='center', out.width="100%"}
...
#Count of districts with higher than average risk
...
In the forecast week (Week `r epiweek_request`), there are `r rel_risk_distr_count` counties with higher than average rest as compared to the same epidemiological week in previous years (`r max(human_exists_yr_min, params$year_modeling_start)` through `r params$year_modeling_end`). This **relative risk** map may be used in conjunction with the **absolute risk** map in the following section. The absolute risk map shows the risk of a county reporting at least one WNV positive human case during this week, and the relative risk map shows if this risk is elevated (or not) as compared to previous years. 

```{r relative_risk_map, echo=FALSE, include=TRUE, fig.align='center', out.width="100%"}
...
## Forecast week WNV absolute risk map

The following map displays the **absolute risk** of predicted positive districts during epidemiological week `r epiweek_request`. This map can be used in conjuction with the **relative risk** map in the previous section. The absolute risk map shows the risk of a county reporting at least one WNV positive human case during this week, and the relative risk map shows if this risk is elevated (or not) as compared to previous years. 

```{r absolute_risk_map, echo=FALSE, include=TRUE, fig.align='center', out.width="100%"}
...
```{r current_year_forecast, echo=FALSE, include=TRUE, fig.align='center', out.width="75%", fig.height=3, fig.width=5}
...
p_cur_yr <- ggplot2::ggplot(data = data_cur_yr) +
  ...
  scale_alpha_manual("", 
                     ...
                     labels = "Range of all models") +
  ggtitle(paste("Statewide model predictions made on", epiyear_request, "week", epiweek_request)) + 
...
#   dplyr::mutate(post_fc_wk = dplyr::if_else(week_epi >= epiweek_request, 
#                                      TRUE,
#                                      FALSE))
# 
...
ArboMAP models are based on 'positive district-weeks', the probability that a district would have at least one human WNV case in a given week. These values can be used to predict a total number of **cases**, shown in the table below. 
...
# DEV <<>> would mean AIC actually useful? Not comparing models here...
...
The following chart shows the model results for the entire modeled period from `r max(human_exists_yr_min, params$year_modeling_start)` through `r params$year_modeling_end`. Years before the forecast year were used for fitting the model. Similar to the previous forecast year chart, the average of all models is shown as a `r colorize("dark red", "darkred")` line, with the range of all models as the shaded ribbon. Predictions for the future (beyond the requested forecast date) are shown as a dotted line. The historical **observed** values are shown in black. 
...
After data processing, the human case data contained a total of `r dx_human_nrow_2clean` rows containing data from years `r human_exists_yr_min` through `r human_exists_yr_max`. Over all years, the state saw a cumulative total of `r state_cases_total` human cases representing `r state_any_cases_total` positive district-weeks from a total of `r dx_human_districts` counties.

```{r human_map, echo=FALSE, include=TRUE, fig.align='center', out.width="75%"}
...
sf_human <- data_sf_orig %>% 
  dplyr::left_join(h_district, by = "arbo_ID")
...
```{r mosquito_pools_2wks_calc, out.width="100%", echo=FALSE, include=TRUE}
...
After data processing, the mosquito pool data contained a total of `r dx_mosq_nrow_2clean` rows, with `r dx_mosq_nrow_3filtered` being in the mosquito modeling period, containing data from years `r mosq_exists_yr_min` through `r mosq_exists_yr_max` with reporting from a total of `r dx_mosq_districts` counties. 

In the forecast year to date, there were `r dx_mosq_nrow_maxyr` pools reported from `r dx_mosq_districts_maxyr` counties. Of these pools `r mosq_pos_num_maxyr` (`r mosq_pos_perc_maxyr`%) were reported WNV positive. 
...
```{r mosquito_pools_2wks, out.width="100%", echo=FALSE, include=TRUE}
...
```{r mosquito_pools_posrate, out.width="100%", echo=FALSE, include=TRUE, fig.height=3}
...
# [DEV] While nonstrat and strat code is very similar, 
# keeping separate because in [V3] strat models did NOT show this graph
# and intial runs show the actual rates off from an average of strata,
# so perhaps stratified models need to be faceted instead ? [DMN]
# [DEV] <<>>
...
if (mosquito_model_clean %in% mosq_nonstrat_models) {
  ...
  #DEV perhaps needs to facet by strata instead??
...
After processing, weather data existed from `r min(data_env$date_obs, na.rm = TRUE)` through `r max(data_env$date_obs, na.rm = TRUE)` for `r data_env$arbo_ID %>% unique() %>% length()` counties. 

The report parameters set the two environmental predictor variables as `r params$predictor_var1` and `r params$predictor_var2`. The following two graphs show the median state-wide observed weather variables for the forecast year, compared to the historical median. Observed values **greater** than the historical are drawn in `r colorize("red", "red")` and observed values **less** than the historical are drawn in `r colorize("blue", "blue")`. Values that cross the historical are in purple. The gray shaded region is a ribbon showing the historical range (min to max). 

```{r weather_charts, echo=FALSE, include=TRUE, fig.align='center', out.width="100%", fig.height=c(2,6), fig.width=c(7,7)}
...
data_env_colors <- data_env_fc_yr %>% 
  ...
  dplyr::mutate(
    ...
    color_var1 = case_when(
      var1_y >= !!rlang::sym(paste0(params$predictor_var1, "_med")) & 
        var1_yend >= !!rlang::sym(paste0(params$predictor_var1, "_med")) ~ "Higher",
      var1_y >= !!rlang::sym(paste0(params$predictor_var1, "_med")) & 
        var1_yend <= !!rlang::sym(paste0(params$predictor_var1, "_med")) ~ "Crosses",
      var1_y <= !!rlang::sym(paste0(params$predictor_var1, "_med")) & 
        var1_yend >= !!rlang::sym(paste0(params$predictor_var1, "_med")) ~ "Crosses",
      var1_y <= !!rlang::sym(paste0(params$predictor_var1, "_med")) & 
        var1_yend <= !!rlang::sym(paste0(params$predictor_var1, "_med")) ~ "Lower",
      TRUE ~ "forgotsomething"),
    #var2
    color_var2 = case_when(
      var2_y >= !!rlang::sym(paste0(params$predictor_var2, "_med")) & 
        var2_yend >= !!rlang::sym(paste0(params$predictor_var2, "_med")) ~ "Higher",
      var2_y >= !!rlang::sym(paste0(params$predictor_var2, "_med")) & 
        var2_yend <= !!rlang::sym(paste0(params$predictor_var2, "_med")) ~ "Crosses",
      var2_y <= !!rlang::sym(paste0(params$predictor_var2, "_med")) & 
        var2_yend >= !!rlang::sym(paste0(params$predictor_var2, "_med")) ~ "Crosses",
      var2_y <= !!rlang::sym(paste0(params$predictor_var2, "_med")) & 
        var2_yend <= !!rlang::sym(paste0(params$predictor_var2, "_med")) ~ "Lower",
      TRUE ~ "forgotsomething")) %>% 
...
p_env_var1_color <- ggplot() + 
  ...
  geom_line(data = data_env_colors,
            aes(x = doy, 
                y = !!rlang::sym(paste0(params$predictor_var1, "_med")),
...
p_env_var2_color <- ggplot() + 
  ...
  geom_line(data = data_env_colors,
            aes(x = doy, 
                y = !!rlang::sym(paste0(params$predictor_var2, "_med")),
                color = "Historical median")) +
  #line segment for year median with different colors
  geom_segment(data = data_env_colors,
               aes(x = var_x, xend = var_xend,
...
#immediate end of landscape if not rotating page
cat("\\blandscape")
```

## Reference map

For `r params$state_name`, the spatial data (shapefile) contained `r unique(data_sf_orig$arbo_ID) %>% length()` counties. 

Alphabetical list of counties: `r unique(data_sf_orig$NAME) %>% sort() %>% stringr::str_c(collapse=", ")`

```{r reference_map, echo=FALSE, include=TRUE, fig.align='center', out.width="100%", fig.width=if (sf_wider) 10 else 7.5}
...
if (!is.null(params_dev$data_sf_orig)){params_dev$data_sf_orig <-"Override data given"}
...
#       As you can't run the child document directly, I want it buried from the user
