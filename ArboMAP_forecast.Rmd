...
params:
  ...
  predictor_var2: "vpd"
  ## mosquito settings: "simpleratio", "AUC", "MIGR", "MII", "stratifiedMIGR", "stratifiedMII"
  ...
  # developer settings
  #dev_settings: !r list() 
  dev_settings: !r list(dev_write_output = TRUE, model_evaluation = TRUE, out_folder = file.path("dev", "20220323"))
...
## <<>><<>> DEV: summary things used to display
...
```{r absolute_risk_map, echo=FALSE, include=TRUE, fig.align='center', fig.width=7}
...
```{r current_year_forecast, echo=FALSE, include=TRUE, fig.align='center', fig.width=7}
...
```{r positives_to_cases, echo=FALSE, include=TRUE, fig.align='center', fig.width=7}
...
pos_case_preds_ave <- pred_yrs_pre_yr %>% 
  ...
  dplyr::mutate(
    ...
    #exact fit years all models are same, range and sd is 0. Changing display
    range_pred_cases = dplyr::if_else(year_epi %in% yrs_mir_imputed, 
                               round_any(ave_tot_pred_cases, 1, ceiling) %>% 
                                 as.character(),
                               range_pred_cases),
    ave_sd_pred_cases = dplyr::if_else(year_epi %in% yrs_mir_imputed,
                                paste0(round_any(ave_tot_pred_cases, 1, ceiling), 
                                       " (NA)"),
                                ave_sd_pred_cases %>% 
                                  as.character()))
...
knitr::kable(pos_case_preds_ave %>%
             ...
             col.names = c("Year", "Predicted positive district-weeks", "Average estimated cases (standard dev)", "Range of estimated cases"),
             digits = 0, align = "c")

#<<>> DEV Make sure to add explanation of exact fit years in text!
# <<>> check that exact fit years is in fact what's going on here.....
# may be created this model with everything but latest year
# ... just show latest year here, and everything with explanation in appendix?
# <<>> DEV
...
```{r mosquito_pools, fig.width=7, echo=FALSE, include=TRUE}

#Creates summaries of the input mosquito data: 4 parts
...
# If data does NOT exist in the current forecast epiweek, then 'past two' will the two epiweeks PRIOR to this week
...
# DEV <<>> Remember to add explanation in report text, so that it's clear WHY these dates 
...
#<<>> DEV
#put in text paste0("Range of dates present in data within two week range: ", mosq_2wks_dat_dts_str)
...
if (mosquito_model_clean %in% mosq_nonstrat_models) {
  ...
  #DEV <<>> Looks off when averaged by strata
  # perhaps needs to facet by strata instead??
...
```{r weather_charts, echo=FALSE, include=TRUE, fig.align='center', fig.width=7, fig.height=c(3,4,3,5)}
...
### OPTION 2
#DEV attempt at segments for above/below median
...
```{r human_map, echo=FALSE, include=TRUE, fig.align='center', fig.width=7}
...
h_district <- data_human %>% 
  ...
  dplyr::mutate(case_count = if_else(is.na(case_count), 0L, case_count))
...
# 
# prev_human_yr <- if_else(epiyear_request - 1 <= params$year_human_end &
#                      epiyear_request - 1 <= max(data_human$year_epi, na.rm = TRUE),
#                    #params$year_human_end if data exist
#                    if_else(params$year_human_end <= max(data_human$year_epi, na.rm = TRUE),
...
p_ref <- ggplot() +
  ...
  ggrepel::geom_text_repel(data = sf_point_ref,
                           ...
                           point.padding = 0) +
  #better colors
  viridis::scale_fill_viridis(alpha = 0.3, option = "turbo", begin = 0.1, end = 0.9, discrete = TRUE) +
  theme_arbo_map +
