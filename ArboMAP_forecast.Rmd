...
In the forecast week there are `r rel_risk_distr_count` counties with higher than average rest as compared to the same epidemiological week in previous years (`r max(human_exists_yr_min, params$year_modeling_start)` through `r params$year_modeling_end`). 
...
\vspace{-5truemm}
```{r human_map, echo=FALSE, include=TRUE, fig.align='center', out.width="70%"}
...
To compare the epicurve of human cases in each years, the heatmap below shows when in each year the cases occurred. 

\vspace{-5truemm}
```{r human_heatcurves, echo=FALSE, include=TRUE, fig.align='center', out.width="80%"}
