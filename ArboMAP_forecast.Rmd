...
# county_sf: sf object of counties for state (overrides file_county_sf)
...
if (mosquito_model_clean %in% mosq_nonstrat_models){
  ...
  prep_mir_filler <- data_combined_prep %>% 
    ...
    dplyr::mutate(mir_impute = (is.na(mir_stat_ctr) & 
                                  year_epi >= year_modeling_start &
                                  year_epi <= year_modeling_end))
...
`r if(nrow(dx_human_unmatched) > 0){paste0("The human case data entries that were unmatched to spatial data during processing are in the table below. Please check for mispellings in the original file.")}`
...
After data processing, the mosquito pool data contained a total of `r dx_mosq_nrow_2clean` rows, containing data from years: `r knitr::combine_words(human_exists_yr_list)`. Overall during this time frame, a total of `r length(dx_mosq_counties)` counties reported mosquito data. 
...
Modeling was done from `r year_modeling_start` through `r year_modeling_end`. Years without mosquito data during these years are assumed to have average mosquito infection rates. This allows us to estimate relationships with environmental data even when mosquito data are not available. There were `r length(yrs_mir_imputed)` years where mosquito infection rates were imputed. `r if(length(yrs_mir_imputed)>0){paste0("These years are: ", yrs_mir_imputed %>% sort() %>% knitr::combine_words(), ".")}` In modeling years where sufficient mosquito data were present, the mosquito infection statistic was created using the model specified in the input parameter: `r mosquito_model_clean`. 
