...
```{r data_env_latest_history, echo=FALSE} 
...
```{r data_env_anomalization, echo=FALSE} 
...
cases_before_fc_wk <- data_combined_prep %>% 
  dplyr::filter(TBC_mod_wk_dt_week < date_request_week) %>% 
  pull(case_count) %>% 
  sum(na.rm = TRUE)
...
if (mosquito_model_clean %in% mosq_nonstrat_models){
  ...
  prep_mir_filler <- data_combined_prep %>% 
    ...
    dplyr::left_join(mosq_mir, by = c("mod_wk_dt_yr" = "year_obs")) %>% 
...
#     could overwrite old one, but creating new temporarily [DMN]
...
data_combined <- data_combined_prep %>% 
  ...
  dplyr::left_join(data_env_lag,
                   by = c("arbo_ID", "date_week" = "date_obs")) %>% 
  as.data.frame()
...
#Function from plyr with no dplyr equivalent (yet)
...
for (this_model in model_names){
  ...
  data_combined$pred[data_combined$TBC_mod_wk_dt_week < human_wk_min] <- 0
  data_combined$pred[data_combined$TBC_mod_wk_dt_week > human_wk_max] <- 0
  ...
  pred_prep <- data.frame(arbo_ID = data_combined$arbo_ID,
                          date_week = data_combined$date_week,
