...
env_mod_date_start <- as.Date(paste0(params$year_modeling_start, "-01-01"), 
                                     #lag length days before modeling start
                                    format = "%Y-%m-%d") - lubridate::days(params$lag_length)
...
`r if(length(env_missing_dates) > 0){paste0("ArboMAP uses weather data from ", env_mod_date_start, ", which is the environmental lag length days (", params$lag_length, ") prior to January 1 of the modeling start year (", params$year_modeling_start, ") through the forecast date. The following days were not found in the dataset from : ", env_missing_dates %>% sort() %>% stringr::str_c(collapse=", "), ". Missing dates, e.g. recent dates that are not yet available are filled in with the historical mean for that day of year.")}`
