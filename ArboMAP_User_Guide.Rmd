...
author: "User's Guide   \nJustin K. Davis and Michael C. Wimberly (mcwimberly@ou.edu)   \nGeography and Environmental Sustainability, University of Oklahoma"
date: "Version 3.2 Updated June 29, 2021"
output: 
  pdf_document:
    toc: true
    toc_depth: 2
...
ArboMAP is a program used to model and predict cases of vector-borne disease. It has been developed and tested with human cases of the mosquito-borne West Nile virus. However, ArboMAP can be adapted to work with any data sets that meet the following conditions:
...
The ArboMAP user guide consists of four main sections. The first section describes how to install the necessary software needed to run the system and the steps that need to be taken in first time setting up. The second section describes how to obtain your own weather, vector, and human data. The third section describes the steps that need to be taken each time a new forecast is generated. The final section describes the outputs produced by ArboMAP and explains how to interpret the forecast charts and maps.
                                                                                                                                                                   
## Background about WNV and modeling
...
For Mac user, please click on the lick \"Download R for macOS.\" Choose latest released R (R-4.1.0.pkg in 06/21/2021). Use the default settings for the installer.
...
Here we assume you have already installed the latest R and Rstudio IDE. Now you are ready to install the *rmarkdown* package in R. Please type following code in your Console window:
**install.packages('rmarkdown')**
Or you can click **Packages** and **Install** and then search *rmarkdown* to install it.

```{r echo=FALSE, out.width='80%'}
knitr::include_graphics('pictures for setup document/Install rmarkdown2.PNG')
```

Since we want to generate PDF output, you will need LaTeX. Instead of downloading and installing LaTeX and MiKTeX, for R Markdown users, a easier way is installing **TinyTex**(**[https://yihui.name/tinytex/](https://yihui.name/tinytex/)**) by typing in your Console window:
**install.packages('tinytex')**
Or you can use the **Packages** and **Install** to install *tinytex' too. Tinytex can help you automatically install missing LaTeX packages when you are knitting R markdown documents to PDF.

## Downloading ArboMAP 

Please download the latest ArboMAP package from EcoGRAPH lab's github repository (**[https://github.com/EcoGRAPH/ArboMAP](https://github.com/EcoGRAPH/ArboMAP)**) and unzip the folder. 
```{r echo=FALSE, out.width='80%'}
knitr::include_graphics('pictures for setup document/Download ArboMap2.PNG')
```

ArboMAP requires a variety of files to run, and the following directory structure is the easiest way to organize everything that is necessary. If you download the repository and run the main ArboMAP code in RStudio using `ArboMAP.Rproj`, you will draw from data sources already in place and will not need to modify the contents of these directories. By default, ArboMAP will expect the following folders to be subfolders of the same home directory of your RStudio project.
...
* **pictures for setup document**: Contains images necessary to generate the user guide, but not required to run the main ArboMAP

Please double click **ArboMAP.Rproj**, which is a R Project file. The default file path used by this project file is the home directory of your ArboMAP folder. So you don't have to change or set up new directory. Then you can single click **ArboMAP Main Code.Rmd** in **Files** window to load the main code for your prediction. 

## Files and settings

The **ArboMAP_Main_Code.Rmd** file, which you will open in RStudio, contains a header (shown below) with settings that tells the program where to find the various pieces of data used to make predictions. Errors in these filenames are a common reason that the code will refuse to run. 

```{r echo=FALSE, out.width='80%'}
knitr::include_graphics('pictures for setup document/ArboMap Project3.PNG')
```
...
* `resample_mosquito` indicates whether a resampling procedure should be used to construct realistic seasonal time series based on seasonal totals. Default setting = FALSE. Please do not change default setting without consultation with the EcoGRAPH group.
...
# Part II: Obtaining Your Own Data

## Obtaining gridMET data
...
ArboMAP reduces all names by deleting \"County\" and \"Parish\", removing all spaces from the district name, and putting the name in lowercase. This gives the greatest chance that all places will be recognized in all data sources. However, incorrect spellings and invalid date formats (e.g. DD/MM/YYYY instead of MM/DD/YYYY) cannot be fixed by the program and will result in errors.

You will need to be sure that the shapefile you use contains a "NAME" field with human-readable names.

# Part III: Running ArboMAP Forecasts

Before you run the model during the WNV season, you will want to be sure you have the most up-to-date weather and mosquito data. These updates will need to be performed every time you want to make an updated prediction.

## Updating the gridMET weather data

Follow exactly the process described above to ensure that your weather data is updated to current day. The default setting is downloading weather data from one month early. All of the new files can simply be added to the weather data folder and the software will automatically eliminate any duplicate values for format the full time series. NOTE: you don't need manually edit any weather data.
...
When new mosquito testing data are available, they need to be added to the existing mosquito data file, which must be updated and replaced in the appropriate data folder. In situations where mosquito data are managed in a database system, this can typically be accomplished via a new database query. Otherwise, the new mosquito observations will need to be appended to the mosquito data file. **NOTE: ALL mosquito data should be in the one csv file.**

## Running the forecasts

In the main directory of the RStudio project, there is a Rmarkdown file called \"ArboMAP_Main_Code.Rmd\". This will do all of the modeling calculations and will produce a PDF report to summarize the results.

Once you have all the necessary data and everything is set up and formatted correctly, all you need to do is: 

1) open the `ArboMAP_Main_Code.Rmd` script, 

2) make any necessary changes to the model parameters, and 

3) execute the model run (called \"knitting\" here). 

There are two different ways to modify the model parameters. The BEST way is to select the **`Knit with Parameters`** option from the `Knit` pull-down menu in RStudio. This will bring up a menu that displays all of the default parameter settings. The user can then choose to modify some of these parameters and click the Knit button in the pop-up menu. ArboMAP is then run using the modified parameters, but no changes are made to the underlying .Rmd file. (The second way is to directly overwrite the parameters in the first section of the .Rmd file. To save the modified parameters, you will need to first either overwrite the existing .Rmd file or save it using a new file name.) 

```{r echo=FALSE, out.width='80%'}
knitr::include_graphics('pictures for setup document/Running main code.PNG')
```
```{r echo=FALSE, out.width='80%'}
knitr::include_graphics('pictures for setup document/Running main code2.PNG')
```
