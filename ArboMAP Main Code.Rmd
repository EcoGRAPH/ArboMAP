...
splineevolution <- "seasonallyvarying"
...
minmaxweekquants <- quantile(human$weeknum, probs=c(0, 1), na.rm=TRUE)
absminobsweek <- minmaxweekquants[1]
absmaxobsweek <- minmaxweekquants[2]
...
In the graph below, positive districts by week are shown over all years.
...
# if we need a seasonally-varying distributed lag, create those terms here
if (splineevolution == "seasonallyvarying") {
  
  fullcasemat$doymat <- fullcasemat$lag
  fullcasemat$doymat[,] <- fullcasemat$doy

  #fullcasemat$cendoymat <- fullcasemat$lag
  #fullcasemat$cendoymat[,] <- fullcasemat$cendoy
...
tempdf <- tempdf[tempdf$week >= absminobsweek,]
tempdf <- tempdf[tempdf$week <= absmaxobsweek,]
...
tempdf$newwe <- (tempdf$week - absminobsweek) / (absmaxobsweek - absminobsweek + 1)
...
tempdf <- tempdf[tempdf$weeknum >= absminobsweek,]
tempdf <- tempdf[tempdf$weeknum <= absmaxobsweek,]
...
termsum <- termsum[termsum$weeknum >= absminobsweek,]
termsum <- termsum[termsum$weeknum <= absmaxobsweek,]
...
fullcasemat$weeknum <- as.numeric(format(fullcasemat$weekstartdate, "%U"))
tempdf   <- fullcasemat[fullcasemat$weeknum >= absminobsweek,]
tempdf   <- tempdf[tempdf$weeknum <= absmaxobsweek,]
