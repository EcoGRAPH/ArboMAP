...
minhumanbelieveyear <- 2004
maxhumanbelieveyear <- 2018
minmosqbelieveyear  <- 2004
maxmosqbelieveyear  <- 2018
minenvbelieveyear <- 1950
maxenvbelieveyear <- 2250
...
weather <- weather[as.numeric(format(weather$date, "%Y")) >= minenvbelieveyear,]
weather <- weather[as.numeric(format(weather$date, "%Y")) <= maxenvbelieveyear,]
...
wnv <- wnv[wnv$year >= minmosqbelieveyear,]
wnv <- wnv[wnv$year <= maxmosqbelieveyear,]
```
...
human <- human[which((human$creationyear >= minhumanbelieveyear) & (human$creationyear <= maxhumanbelieveyear)),]
...
fullcasemat$observed   <- 1*((as.numeric(format(fullcasemat$weekstartdate, "%Y")) >= minhumanbelieveyear)&
                             (as.numeric(format(fullcasemat$weekstartdate, "%Y")) <= maxhumanbelieveyear))
...
totalweather <- expand.grid(district=unique(weather$district),
                              date=seq(from=as.Date(paste(minhumandesiredyear-1, "-01-01", sep=""),
                                                    "%Y-%m-%d"),
                                       to=  as.Date(paste(maxhumanbelieveyear, "-12-31", sep=""),
                                                    "%Y-%m-%d"),
                                       by=1))
...
# handle this with another model later
...
for (curmodel in modelnames) {
  ...
  fullcasemat$pred[fullcasemat$weeknum < humanminobsweek] <- 0
  fullcasemat$pred[fullcasemat$weeknum > humanmaxobsweek] <- 0
...
tempdf2 <- tempdf2[tempdf2$weeknum >= humanminobsweek,]
tempdf2 <- tempdf2[tempdf2$weeknum <= humanmaxobsweek,]
...
thisplot2 <- ggplot(data=tempdf2) +
  ...
  scale_color_manual("", values = mycolors) +
  scale_linetype_manual("", values = mylinetypes) +
  ggtitle(paste("Statewide model predictions in ", maxmosqyear, sep="")) +
  xlab("") + ylab("") +
