...
# model names
modelnames <- c("tp-sv-anom", "cub-fx-nonanom")

# model formulas
modelformulas <- c("cub-fx-nonanom"   = "anycases ~ 0 + district + MIRsummarystat + s(lag, by=var1, bs='cr') + s(lag, by=var2, bs='cr')",
                   "cub-sv-nonanom"   = "anycases ~ 0 + district + MIRsummarystat + s(lag, doymat, by=var1, bs='cr') + s(lag, doymat, by=var2, bs='cr')",
                   "tp-fx-nonanom"    = "anycases ~ 0 + district + MIRsummarystat + s(lag, by=var1, bs='tp') + s(lag, by=var2, bs='tp')",
                   "tp-sv-anom"       = "anycases ~ 0 + district + MIRsummarystat + s(lag, doymat, by=anom_var1, bs='tp') + s(lag, doymat, by=anom_var2, bs='tp') + s(doy, bs='tp')")
...
This instance of ArboMAP is running on a `r ostype` machine, with spline type `r splinetype`, splines that are `r splineevolution` and `r anomalization`.
...
In the graph below, positive districts by week are shown over all years. Only weeks within the two dashed lines are used in modeling, so that the consistently negative weeks outside of this range do not have an effect on the stability of the model.
...
for (curmodel in modelnames) {
  ...
  tempdf <- data.frame(auc = roc(response = tempdf$anycases,
...
tempdf2$est <- round_any(tempdf2$est, 0.01)
...
thisweek <- thisweek[c("district", "pred")]

projected_districts.df <- left_join(projected_districts.df, thisweek,
                                   by="district")
projected_districts.df$pred[is.na(projected_districts.df$pred)] <- min(projected_districts.df$pred, na.rm=TRUE)
```
...
# restrict display to only those months for which there are at least one positive observation
termsum$date <- as.Date(paste(termsum$year, "-01-01", sep=""), "%Y-%m-%d") + (termsum$doy - 1)
#termsum$month <- as.numeric(format(termsum$date, "%m"))
termsum$weeknum <- as.numeric(format(termsum$date, "%U"))

termsum <- termsum[termsum$weeknum >= humanminobsweek,]
termsum <- termsum[termsum$weeknum <= humanmaxobsweek,]

maxyear <- termsum[termsum$year == max(termsum$year, na.rm=TRUE),]
termdf$year <- factor(termdf$year)

thisplot1 <- ggplot() + geom_line(data=termsum, aes(x=weeknum, y=var1inf, group=year), alpha=0.5) +
  geom_line(data=maxyear, aes(x=weeknum, y=var1inf), color="red", size=2) +
  ggtitle(paste("Relative influence of ", var1name, sep="")) +
  xlab("week number") + ylab("relative influence")
thisplot2 <- ggplot() + geom_line(data=termsum, aes(x=weeknum, y=var2inf, group=year), alpha=0.5) +
  geom_line(data=maxyear, aes(x=weeknum, y=var2inf), color="red", size=2) +
...
tempdf <- tempdf[tempdf$year < max(tempdf$year, na.rm=TRUE),]
myhoslemtotal <- hoslem.test(x=tempdf$anycases,
                             y=tempdf$est)$p.value

# do the same on restricted set
fullcasemat$weeknum <- as.numeric(format(fullcasemat$weekstartdate, "%U"))
tempdf   <- fullcasemat[fullcasemat$weeknum >= humanminobsweek,]
tempdf   <- tempdf[tempdf$weeknum <= humanmaxobsweek,]
tempdf   <- tempdf[tempdf$year < max(tempdf$year),]
tempdf   <- tempdf[!is.na(tempdf$anycases),]
tempdf   <- tempdf[!is.na(tempdf$est),]
