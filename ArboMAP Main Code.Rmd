...
thisplot <- ggplot() + geom_line(data=tempdf, aes(x=newdate, y=obs, group=model)) +
  ...
  xlab("") + ylab("") +
  scale_x_continuous(breaks=seq(from=minmosqyear,to=(maxhumandesiredyear+1),by=1),
                     labels=c(seq(from=minmosqyear, to=maxhumandesiredyear, by=1), ""),
                     limits=c((minmosqyear+0.125),(maxhumandesiredyear+1))) +
