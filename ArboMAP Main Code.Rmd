...
humandatafile <- ".\\human case data\\21-02-19 - fake human case data.csv"
...
humanbelievableyears    <- c(2004:2020)
mosquitobelievableyears <- c(2012:2020)
...
weekinquestion <- as.Date("2020-10-01", "%Y-%m-%d")

# where are the weather csv files stored?
weatherpathstr <- ".\\Weather\\"
...
#mosqfile <- "C:\\home\\work\\davis\\WNV repository\\South Dakota\\mosquito\\21-02-12 - example of flat mosquito file.csv"
mosqfile <- "C:\\home\\work\\davis\\WNV repository\\South Dakota\\mosquito\\20-09-24 - mosquito testing records.csv"
mosqfile <- ".\\mosquito data\\okmosqdata - 2020 update - culex only.csv"
...
resamplemosquitodatafile <- "C:\\home\\work\\davis\\WNV repository\\South Dakota\\mosquito\\20-09-24 - mosquito testing records.csv"
...
if (mosqsummarystyle == "AUC") {
  ...
  randeffs <- as.data.frame(dplyr::summarise(group_by(predframe, year),
                                             mosqinfect=sum(pred, na.rm=TRUE)))
  randeffs$year <- as.numeric(as.character(randeffs$year))
...
if (mosqsummarystyle %in% c("MIGR", "MII")) {
  ...
  if (mosqsummarystyle == "MIGR") {
    ...
    randeffs <- data.frame(year=as.numeric(rownames(random.effects(infectglm)$year)),
                           mosqinfect=random.effects(infectglm)$year[,1])
  ...
  if (mosqsummarystyle == "MII") {
    ...
    # predict random effects for all years
    randeffs <- data.frame(year=as.numeric(rownames(random.effects(infectglm)$year)),
                           mosqinfect=random.effects(infectglm)$year[,2])
...
if (imputemissingdistricts) {
  ...
  whichtocopy <- sample(x=1:nrow(human$district),
                        size=nrow(tempdf),
                        replace=TRUE)
