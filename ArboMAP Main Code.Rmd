...
# model names
modelnames <- c("tp-sv-anom", "cub-sv-nonanom")
...
modelformulas <- c("cub-fx-nonanom"   = "anycases ~ 0 + district + MIRsummarystat + s(lag, by=var1, bs='cr') + s(lag, by=var2, bs='cr')",
                   ...
                   "cubfx-fx-nonanom" = "anycases ~ 0 + district + MIRsummarystat + s(lag, by=var1, bs='cr', fx=TRUE, k=6) + s(lag, by=var2, bs='cr', fx=TRUE, k=6)",
...
```{r modelplots, include=TRUE, echo=FALSE}
...
for (curmodel in modelnames) {
  ...
  for (curplot in 1:length(thisplot)) {
    ...
    if (!is.null(thissubplot$main)) {
      ...
      mygg <- ggplot(tempdf) + geom_raster(aes(x=x, y=y, fill=fit)) +
        xlab(thissubplot$xlab) + ylab(thissubplot$ylab) +
