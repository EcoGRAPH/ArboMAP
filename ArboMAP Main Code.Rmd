...
modeltype <- "fixeddfcr"
if (!(modeltype %in% c("fixeddfcr", "thinplate"))) { modeltype <- "thinplate" }
...
humandatafile <- "C:\\work\\arbomap data for mike\\human\\19-01-29 - reconciled human WNV.csv"
...
mosqfile <- "C:\\work\\arbomap data for mike\\mosquito\\19-01-29 - state testing.csv"

# which district stratification scheme are we using?
stratafile <- ".\\strata\\17-04-20 - classified strata - classic.csv"
stratafile <- "C:\\work\\arbomap data for mike\\geographies\\17-04-20 - classified strata - classic.csv"

# where is the districtshapefile
districtshapefile <- ".\\shapefile\\cb_2014_us_county_5m - in EPSG 5070 - only SD.shp"
districtshapefile <- "C:\\work\\arbomap data for mike\\geographies\\cb_2014_us_county_5m - in EPSG 5070 - only SD.shp"
...
```{r mosquitodataprocess2, fig.width=7, fig.height=3, echo=FALSE, include=TRUE}
...
# 
# # delete this!!!
# dailyextr$year <- as.numeric(format(dailyextr$date, "%Y"))
# dailyextr$var1[dailyextr$year == 2019] <- dailyextr$meanvar1[dailyextr$year == 2019]
# dailyextr$var2[dailyextr$year == 2019] <- dailyextr$meanvar2[dailyextr$year == 2019]
# dailyextr$year <- NULL
...
Below, we visualize the distributed lags by stratum.
...
The following two graphs show the influences of the two environmental covariates on estimated risk. The current year (red) is compared to all others (black).
...
```{r totalcases2, include=TRUE, echo=FALSE, fig.width=7, fig.height=7}
fullcasemat <- group_by(fullcasemat, year)
sumcases    <- dplyr::summarise(fullcasemat,
                                totcase = sum(anycases, na.rm=TRUE),
                                sumest  = sum(est, na.rm=TRUE))
tempdf <- sumcases[sumcases$year < 2019,]
ggplot(tempdf) + geom_label(aes(x=sumest, y=totcase, label=year)) +
  geom_abline(slope=1, intercept=0, color="red", linetype=2) +
  xlim(0,max(c(sumcases$totcase,
               sumcases$sumest))) +
  ylim(0,max(c(sumcases$totcase,
               sumcases$sumest))) +
