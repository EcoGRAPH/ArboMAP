...
# humandatafile <- "C:\\work\\arbomap data\\human\\19-01-29 - total human WNV.csv"
...
weathernames <- c("district", "doy", "year", "date", "districtdate", var1name, var2name)
weather <- weather[,weathernames]
...
if (splinetype == "fixeddfcr") {
  ...
  myformula <- "s(lag, by=var2, bs='cr', fx=TRUE, k=6) + s(lag, by=var2, bs='cr', fx=TRUE, k=6)"
}
...
ggplot(lag1df) + geom_line(aes(x=x, y=fit)) +
  ggtitle(paste("distributed lags on ", var1name, sep="")) + xlab("lag")
ggplot(lag2df) + geom_line(aes(x=x, y=fit)) +
...
The model object is summarized below:
