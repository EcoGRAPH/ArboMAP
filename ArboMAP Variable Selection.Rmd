...
variableselectiondir <- ".\\variable selection output\\"
...
for (curmodelnum in 1:nrow(modelstotry)) {
  ...
  tempdailyextr <- dailyextr[c("county","date","var1","var2")]
  tempdailyextr$county <- simplifynames(tempdailyextr$county)
  # fill in missing and future climatological data
  totaldailyextr <- expand.grid(county=unique(tempdailyextr$county),
  ...
  tempdailyextr <- group_by(tempdailyextr, county, doy)
  countydoymean <- dplyr::summarize(tempdailyextr,
                             meanvar1=mean(var1, na.rm=TRUE),
                             meanvar2=mean(var2, na.rm=TRUE))
  tempdailyextr <- ungroup(tempdailyextr)                           
  
  tempdailyextr <- left_join(tempdailyextr, countydoymean,
                     by=c("county","doy"))
  ...
  tempfullcasemat$county <- simplifynames(tempfullcasemat$county)
  
  datalagger <- expand.grid(unique(tempfullcasemat$county),
