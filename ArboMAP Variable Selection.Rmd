...
# fix some of the bad district names, delete what we can't use
#wnv <- wnv[which(wnv$district %!in% c("doh",
#                                    "doh-withers",
#                                    "mitchell-carlson",
#                                    "unknown",
#                                    "")),]
...
distinctcounties     <- length(unique(wnv$district))
districtlist$districtnum <- seq(from=1, to=distinctcounties, by=1)
...
for (curmodelnum in 1:nrow(modelstotry)) {
  ...
  # we need the counties to be in lower case to merge with the weather data
