...
ArboMAP is a set of code written primarily in R, used to model and predict cases of vector-borne disease. Here we consider human cases of the mosquito-born West Nile virus, but ArboMAP will likely work with any data set in which:

* there are multiple (>3) years of infection data from years in which the pathogen can be considered endemic, after any introductory years in which the pathogen is probably exploiting naive populations and should not be considered typical
* the disease has distinct transmission and quiescent seasons, including an initial, yearly exponential growth phase when the pathogen is beginning to spread after a period of no or few cases
* there is reason to believe that incidence responds to measurable environmental indices, probably including temperature and some measure of moisture in the environment (e.g. precipitation or humidity)
* cases of disease are assigned to areal units (states, counties, etc.) and cases are not too rare - every modeled unit should have at least one case over the period of study, as the model will assume an area is permanently immune if no cases have been observed there
* some measure of pathogen in the vector is available; e.g. here we use the rate at which pools of mosquitoes test positive for the virus. 

# What you need to know about WNV and modeling

West Nile virus (WNV) circulates primarily among mosquitoes and birds, but occasionally spills over into human or other hosts. In humans, the majority (~80%) of infections show no symptoms and are not diagnosed except perhaps accidentally during blood donation. Some small proportion of individuals will develop symptoms such as headache, fever, and rash, but in about 1 out of every 100 infected individuals, the disease becomes neuroinvasive and can cause debilitating illness and death.
...
The ArboMAP system was developed to implement this modeling approach via a straightforward workflow. The system uses the R programming language and software environment for statistical computing and graphics along with Google Earth Engine for accessing environmental data. ArboMAP was originally developed to support WNV forecasting in South Dakota, and this guide therefore references specific data formats that are unique to this state. However, it should be straightforward to modify data from other locations into a similar format in order to use them in ArboMAP.
...
To obtain mosquito infection data for South Dakota, you will need an account at **[mosquito.sdstate.edu](https://mosquito.sdstate.edu)**. This account will need to be set up by someone at SDSU. If you work for a mosquito control district or another state that collects these data, then you will be able to modify your CSV files to match what we require here, and the ArboMAP system should run in the same way.

Environmental data will be obtained from the gridMET dataset and downloaded using Google Earth Engine (GEE). GEE is a cloud-based platform for processing satellite remote sensing images and other geospatial datasets. Instead of downloading the data and processing it yourself, GEE uses cloud-based computers and algorithms to do in half an hour what might take a standard desktop computer a week to calculate.
...
If you are associated with the South Dakota Department of Health (SDDOH), you should already have access to up-to-date human case data. If not, check with your group to determine whether there is a data-sharing agreement with the SDDOH. If you are implementing this workflow outside of South Dakota, then it will be up to the you to obtain local mosquito data and put it in the appropriate format.
...
ArboMAP requires weather data to make its predictions. You will use Google Earth Engine (GEE) to download historical weather data beginning a full year before your human case data (e.g. if you begin modeling in 2001, you will need to download all data beginning in 2000). If you want to download all data, it will not hurt the calculations; simply begin in 1950). You will only need to download historical data once; every other time you run the program, you will only need to download the most recent year's weather data from GEE.
...
The CSV file you use should that contains the human case data should have a unique column named \"creationdate\" that is formatted MM/DD/YYYY, and a column named \"county\" that contains standard names for SD counties. Here, any of the following are acceptable: BROOKINGS, Brookings, brookings, Brookings County, BROOKINGS COUNTY, and they will all be reduced to \"brookings\" in the output.

It does not matter which other columns are present, as long as each row contains at least these two values. Cases without either of these values will not be modeled. Below is an example of an acceptable (fake) data file. Note that other columns are present, but they are not used by ArboMAP.

```{r echo=FALSE, out.width='80%'}
knitr::include_graphics('pictures for setup document/example of human case data.PNG')
```

Save this file in its own directory. By default this is \"D:/ArboMAP/human/human case data.csv\".
...
### \ \ \ \ \ Obtaining the mosquito data

South Dakota users will be given a login for **[mosquito.sdstate.edu](http://mosquito.sdstate.edu)** by collaborators at SDSU. Log in and navigate to Mosquito Data. There are two databases to download - the active data view, which contains tested pools from the current year, and the archive data view, which contains tested pools from prior years.

To download data from the current year, click on \"Testing Records\" under Active Data Views and scroll to the bottom of the page. There will be a small orange button labeled \"CSV.\" Click on this to download the mosquito infection data for this year. Save this file in the directory dedicated to the mosquito infection data with a name that indicates it is the active view; for example  \"D:/ArboMAP/mosquito/active_testing.csv\".

```{r echo=FALSE, out.width='100%'}
knitr::include_graphics('pictures for setup document/active_testing.PNG')
```

Note that this file must include a column named \"Collection Date\" in MM/DD/YYYY format, a \"Species\" column, a \"WNV Result\" column coded as Positive or Negative, a \"Trap\" location, and a \"County\" column. \"Trap\" is needed for backwards compatibility with earlier versions of the database.

To download data from previous years, click on \"Testing Records\" under the Archive Data Views and scroll to the bottom of the page to find the CSV button. This can take a little longer to compile since it includes all past mosquito infection data. Save this file as \"D:/ArboMAP/mosquito/archive_testing.csv\".

```{r echo=FALSE, out.width='100%'}
knitr::include_graphics('pictures for setup document/archive_testing.PNG')
```

This file must include a column named \"WNV Result,\" which is either 0, which means a Negative pool, or 1, which means a Positive pool. There must be \"County,\" \"Trap,\" and \"Collection Date\" columns in MM/DD/YYYY format. There must be a \"Species.Old\" column. We only model mosquitoes labeled \"Culex tarsalis\".

If you are downloading these files from the mosquito.sdstate.edu website rather than creating them on your own, these should be simple downloads. However, it is possible that something has changed in the format of the download, and we recommend that you check the formats of the files.
...
Note that in 2015, Shannon County, SD became Oglala Lakota County. There are similar changes in other states. You are welcome to use either name in the human case data file, but the output will use \"shannon\" for backwards compatibility. If the output is to be distributed to the public, this should be changed.
