...
The `modelformulas` object contains a list of formulas used to model the data. Currently, the naming convention describes whether cubic (cub) or thin-plate (tp) splines are used. Then, the distributed lags are either fixed (fx) or seasonally-varying (sv). Then, we either use anomalized (anom) or non-anomalized (nonanom) environmental data. By choosing the names of the models in `modelnames`, you can run either all the models or any combination. Currently, the default is to run two: cub-sv-nonanom, which is similar to our earliest models, and tp-sv-anom, which uses the most up-to-date statistical model.
