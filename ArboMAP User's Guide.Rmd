...
* `case matrix with estimates` will hold a single file at the end of processing, which contains all the numerical data used for modeling and the estimates for each district and week. This can be used for additional statistical analyses if desired.
...
* `mosquito data` contains the mosquito (or other vector) pool infection data files.
* `shapefile` contains the files used to map risk. This is by default South Dakota, but these files may be replaced with any similar SHP file that references district names in a "NAME" column. We suggest the use of TIGER products for any application in the US.
* `strata` contains a file that tells the model how to split the shapefile into different geographic regions for modeling vector infection rates.
* `weather data` contains a number of CSV files holding the weather data. This folder should only contain weather data, as any additional files placed here will be included as if they were weather data, and will probably cause errors in processing.
...
* `var1name` and `var2name` indicate which two weather variables you'd like to use as predictors in the model. By default, these are set to mean daily temperature from the gridMET set (tmeanc) and the vapor pressure deficit (vpd). If you are not certain which variables you would like to use, read the selection below on selection of covariates.
...
ArboMAP requires weather data to make its predictions. You will use likely Google Earth Engine (GEE) to download historical weather data beginning a full year before your human case data (e.g. if you begin modeling in 2001, you will need to download all data beginning in 2000). If you want to download all available historical data, it will not hurt the calculations; simply begin in 1950). You will only need to download historical data once; every other time you run the program, you will only need to download the most recent year's weather data from GEE.
...
Next are the model outputs. For each week of a year, the model estimates what proportion of districts will report at least one human case based on fit to historical data. This is displayed first on a statewide basis - red are estimates and black are observations. For the year in which you're making predictions, observed (black) should be constantly zero. Red should fit historical observations reasonably well - if there are major deviations, then it is likely that some data, probably the human or mosquito data, are in the wrong format.
...
Next are the absolute probabilities of each district reporting at least one human case during the week in question. If the district is darkest blue, then the model says there is no probability of reporting a case during the week you've selected. This is often the case in the early or late seasons, especially in the lower-population districts. If a district is brightest red, then the model says the district will definitely report at least one human case this week.
...
Combining the two maps above, we conclude that it's still early in the season and absolute risk is low, but there are indications that risk is higher than usual in most districts. You will want to interpret the temperature, humidity, and mosquito infection charts to understand exactly why this is the case.

# Selecting covariates to use

ArboMAP also includes an `ArboMAP Variable Selection.Rmd` file, with settings that are almost exactly the same as the `ArboMAP User's Guide.Rmd` file. However, this file will run through all available pairs of covariates and rank models by the Akaike information criterion, which measures model fit and parsimony. If all of the settings are correct, then opening this file and pressing ctrl+shift+K will run through all models and report a table like the following. The model with the lowest AIC is usually considered best, and the two variables for that model (in this case tmeanc and vpd) should be used as var1name and var2name in the `ArboMAP Main Code.Rmd' file.

The area under the curve (AUC) or c-statistic is presented as well. This number, which ranges from 0 (all attempts at discriminating among positive and negative district-weeks failed) to 1 (perfect discrimination), should not be used for variable selection, but is presented so that model discriminatory power can be estimated. Especially in areas where there are several months of no human cases (e.g. during the winter), this number should be high (> 0.90). The AIC only allows us to measure relative differences among models, but the AUC measures whether the models are worthwhile at all.

```{r echo=FALSE, out.width='60%'}
knitr::include_graphics('pictures for setup document/example of variable selection.PNG')
```
