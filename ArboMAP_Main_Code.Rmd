...
params:
  ...
  human_file: "simulated human case data.csv"
  mosquito_file: "simulated mosquito tests.csv"
  stratification_file: "example SD strata.csv"
  predictor_v1: "tmeanc"
  predictor_v2: "vpd"
  mosquito_model: "stratifiedMIGR"
  min_human_year: 2004
  max_human_year: 2017
  min_mosquito_year: 2012
  max_mosquito_year: 2018
  min_weather_year: 2000
  max_weather_year: 2018
  min_desired_year: 2004
  max_desired_year: 2018
...
#rm(list=ls()) Can't do this or it will clear the params as well.
...
packages <- c("reshape2", "readr", "dplyr", 
              "ggplot2", "gridExtra", "shiny",
...
# state to model
statetomodel <- params$state_code

# Note - I think this line of code can now be deleted MCW 31MAY2021
# do we have a stratification file?
#stratafile <- "./strata/example SD strata.csv"

# where are the data located?
#humandatafile <- "./human case data/ok human case data.csv"
...
# run some subset of the models

model_in <- readr::read_csv("models.txt", col_names = FALSE)
modelformulas <- dplyr::pull(model_in, 2, name = 1)
modelnames <- names(modelformulas)
#modelnames <- c("cub-fx-nonanom")

# where are the human data located?
humandatafile <- file.path(params$data_path, "human case data", params$human_file)

# should we impute human cases in districts where there aren't enough to
# support the estimate of a district-specific intercept? i.e. should we
# put a floor on estimated risk?
imputemissingdistricts <- FALSE

# set up dates to constrain the human, mosquito, and weather data
humanbelievableyears    <- c(params$min_human_year:params$max_human_year)
mosquitobelievableyears <- c(params$min_mosquito_year:params$max_mosquito_year)
envbelievableyears      <- c(params$min_weather_year:params$max_weather_year)
...
minhumandesiredyear <- params$min_desired_year
maxhumandesiredyear <- params$max_desired_year

# cull some proportion of the outlying human cases
humancasealpha <- params$case_trim_alpha

# which week are we producing the graphs for?
weekinquestion <- as.Date(params$forecast_date, "%Y-%m-%d")

# where are the weather csv files stored?
weatherpathstr <- file.path(params$data_path, "weather data")

# what is the name of the summary file to be created?
weathersummaryfile <- "weathersummary.txt"

# which variables do you want to use?
# vpd, rmean, pr
var1name <- params$predictor_v1
var2name <- params$predictor_v2

# where are the mosquito test files located?
mosqfile <- file.path(params$data_path, "mosquito data", params$mosquito_file)

# declare which summary statistic will be used to create the MIRsummarystat
mosqsummarystyle <- params$mosquito_model
if (!(mosqsummarystyle %in% c("simpleratio",
                              "MIGR",
                              "stratifiedMIGR",
                              "MII",
                              "stratifiedMII",
  ...
  mosqsummarystyle <- "simpleratio"
  
}

# if we have simpleratio data, do we need to resample so that one of the other
# methods of summary can be used? If we do want to resample, which data do we use?
resamplemosquitodata <- params$resample_mosquito
resamplemosquitodatafile <- file.path(params$data_path, "mosquito data", params$resample_file)

# to which two other years do we want to compare the current year's predictions?
compyear1 <- params$comp_year_vis1
compyear2 <- params$comp_year_vis2
...
# set up lag length for environmental data
laglen   <- 181
```

# Data used for predictions

ArboMAP is set to use weather data from the years `r envbelievableyears`. If weather are data are necessary outside of this range (e.g. predicting into the far future) then the program will use daily averages from observed years to fill in times where data are not available. A year appearing in this list does not mean that those data are actually available to ArboMAP - solely that the program will permit their use if they are present. These data need to be actually present in the weather directory, but observations will be discarded if ArboMAP has not been instructed to use those years.

ArboMAP is set to use mosquito infection data from the years `r mosquitobelievableyears`. Years without mosquito data are assumed to have average mosquito infection rates. Just because a year is present in this list does not imply that those data have actually been made available to ArboMAP - just that ArboMAP will use those data if they are available. Even if there were no positive pools in a given year, if there were any pools tested then the data will be useful; zero infection rates do predict low-risk years and should be used.

ArboMAP is set to use human infection data from the years `r humanbelievableyears`. This list **should not** include the year in which human cases are being predicted; e.g., if you are predicting for 2020, then 2020 should not be in this list. If, however, you are running historical estimates (as opposed to future predictions), then it may be reasonable to let ArboMAP use these data. Even if there were no cases in a year, if zero was the actual reported number of cases, then the human case data for that year should be used.

If you use human data in a year and either 1) do not use the mosquito data from that year or 2) are not able to obtain an estimated mosquito infection growth rate for that year (e.g. too few mosquito pools reported), then ArboMAP will substitute the mosquito data with a single constant for risk that year. This allows us to estimate relationships with environmental data even when mosquito data are not available, but it means the fit in that year will be more accurate than would have been permitted by the use of mosquito infection data.
...
for (i in 1:length(weatherlist)) {
  ...
  if (weatherlist[[i]] != weathersummaryfile) {
    tempdf <- read.csv(file.path(weatherpathstr, weatherlist[[i]]))
...
weather <- distinct(weather, districtdate, .keep_all=TRUE)
...
for (curcol in c(var1name, var2name)) {
  ...
  # replace any missing
  temppred <- predict(tempmod, newdata=weather)
  weather[is.na(weather[,curcol]), curcol] <- temppred[is.na(weather[,curcol])]
  
  # calculate the anomaly in a different way
  weather[,paste("anom_", curcol, sep="")] <- weather[,curcol] - temppred
  weather[is.na(weather[,paste("anom_", curcol, sep="")]),paste("anom_", curcol, sep="")] <- 0
...
# if we need to resample the data, then do it right now
if (resamplemosquitodata) {
  
  # read in the flat file
  tempdf <- read.csv(mosqfile, stringsAsFactors=FALSE)
  
  # read and format the resampling file
  wnvresamplefile <- read.csv(resamplemosquitodatafile)
  wnvresamplefile$col_date <- as.Date(wnvresamplefile$col_date, "%m/%d/%Y")
  wnvresamplefile$doy      <- as.numeric(format(wnvresamplefile$col_date, "%j"))
  
  positivedoys <- wnvresamplefile$doy[wnvresamplefile$wnv_result == 1]
  negativedoys <- wnvresamplefile$doy[wnvresamplefile$wnv_result == 0]
  
  # create and fill up the expanded file
  wnv <- data.frame()
  for (i in 1:nrow(tempdf)) {
    
    positives <- tempdf$positives[i]
    negatives <- tempdf$total[i] - tempdf$positives[i]
    ...
    if (positives > 0) {
      ...
      tempdf1 <- data.frame(doy = sample(x=positivedoys,
                                         ...
                                         replace=TRUE),
                            year = tempdf$year[i],
                            district = district_shapes$district[1],
                            wnv_result = 1)
    ...
    wnv <- bind_rows(wnv, tempdf1, tempdf0)
    
  }
  
  wnv$col_date <- as.Date(paste(wnv$year, wnv$doy, sep="-"),
                          "%Y-%j")
...
if (mosqsummarystyle %in% c("simpleratio")) {
  ...
  numpos <- NA
  perpos <- NA
  diagnostic_mosquitodistricts <- NA
  
  
}
if (mosqsummarystyle == "AUC") {
...
if (mosqsummarystyle %in% c("MIGR", "MII", "stratifiedMIGR", "stratifiedMII")) {
...
if (mosqsummarystyle == "simpleratio") {
  
  wnv$year <- wnv$col_year
  randeffs <- as.data.frame(dplyr::summarise(wnv, year),
                            totpos = sum(wnv_result, na.rm=true),
                            tottest = n())
  
  randeffs$MIRsummarystat <- randeffs$totpos / randeffs$tottest
  randeffs$year <- as.numeric(as.character(randeffs$year))
...
if (mosqsummarystyle == "AUC") {
  ...
  # make sure all the observations have a year
  wnv <- wnv[!is.na(wnv$year),]
  wnv$year <- factor(wnv$year)
  
  # run a random effect model on orthogonalized data
  infectglm <- glmer(wnv_result ~ poly(dminus,2) + (poly(dminus, 2)|year),
                     family=binomial(),
                     data=wnv)
  
  # create a data frame to store the calculations for the aucs
  predframe <- expand.grid(year = unique(wnv$year),
                           dminus = seq(from=min(wnv$dminus, na.rm=TRUE),
                                        to  =max(wnv$dminus, na.rm=TRUE),
                                        length.out=100))
  predframe$pred <- predict(infectglm,
                            newdata=predframe,
                            type="response")
  
  # calculate the AUCs
  randeffs <- as.data.frame(dplyr::summarise(group_by(predframe, year),
                                             MIRsummarystat=sum(pred, na.rm=TRUE)))
  randeffs$year <- as.numeric(as.character(randeffs$year))
  
}
if (mosqsummarystyle %in% c("MIGR", "MII")) {
  ...
  if (mosqsummarystyle == "MIGR") {
    
    # predict random effects for all years
    randeffs <- data.frame(year=as.numeric(rownames(random.effects(infectglm)$year)),
                           MIRsummarystat=random.effects(infectglm)$year[,1])
    
  }
  if (mosqsummarystyle == "MII") {
    
    # predict random effects for all years
    randeffs <- data.frame(year=as.numeric(rownames(random.effects(infectglm)$year)),
                           MIRsummarystat=random.effects(infectglm)$year[,2])
...
if (mosqsummarystyle %in% c("stratifiedMIGR", "stratifiedMII")) {
  ...
  # read stratum file
  stratafile <- file.path(params$data_path, "strata",
                               params$stratification_file)
  ...
  wnv$stratumyear <- factor(paste(wnv$strata, wnv$year, sep="-"))
    
  # run a random effect model on orthogonalized data
  infectglm <- glmer(wnv_result ~ 1+dminus+
                       (0+1|stratumyear) +
                       (0+dminus|stratumyear),
                     family=binomial(),
                     data=wnv)
  wnv$est <- predict(infectglm, newdata=wnv, type="response")
  
  if (mosqsummarystyle == "stratifiedMIGR") {
    
    # predict random effects for all years
    randeffs <- data.frame(stratumyear=rownames(random.effects(infectglm)$stratumyear),
                           MIRsummarystat=random.effects(infectglm)$stratumyear[,1])
    randeffs$year <- as.numeric(sapply(strsplit(randeffs$stratumyear, "-"), function(x) x[2]))
    randeffs$stratum <- sapply(strsplit(randeffs$stratumyear, "-"), function(x) x[1])
    
  }
  if (mosqsummarystyle == "stratifiedMII") {
    
    # predict random effects for all years
    randeffs <- data.frame(stratumyear=rownames(random.effects(infectglm)$stratumyear),
                           MIRsummarystat=random.effects(infectglm)$stratumyear[,2])
    randeffs$year <- as.numeric(sapply(strsplit(randeffs$stratumyear, "-"), function(x) x[2]))
    randeffs$stratum <- sapply(strsplit(randeffs$stratumyear, "-"), function(x) x[1])
...
if (mosqsummarystyle %in% c("MIGR", "MII", "AUC", "simpleratio")) {
  
  # center these statistics
  randeffs$MIRsummarystat <- randeffs$MIRsummarystat - mean(randeffs$MIRsummarystat, na.rm=TRUE)
  randeffs$MIRsummarystat[is.na(randeffs$MIRsummarystat)] <- 0
  
  # plot
  thisplot <- ggplot(randeffs) + geom_line(aes(x=year, y=MIRsummarystat)) +
  ...
  plot(thisplot)
  write.csv(randeffs, file="randeffs.csv")
...
if (mosqsummarystyle %in% c("stratifiedMIGR", "stratifiedMII")) {
  ...
  # plot
  thisplot <- ggplot(randeffs) + geom_line(aes(x=year, y=MIRsummarystat, group=stratum, color=stratum)) +
    geom_abline(slope=0, intercept=0, linetype=2) +
    scale_x_continuous(breaks=minmosqbelieveyear:maxmosqbelieveyear) +
    theme(axis.text.x=element_text(angle=45, hjust=1),
          panel.grid.minor=element_blank(),
...
# do we need to create data to put 
if (imputemissingdistricts) {
  
  missingdistricts <- diagnostic_shapefiledistricts[!(diagnostic_shapefiledistricts
                                                      %in% human$district)]
  
  tempdf <- data.frame(district=missingdistricts)
  
  whichtocopy <- sample(x=1:nrow(human),
                        size=nrow(tempdf),
                        replace=TRUE)
  
  tempdf$date <- human$date[whichtocopy]
  tempdf$chardate <- human$chardate[whichtocopy]
  tempdf$creationyear <- human$creationyear[whichtocopy]
  tempdf$creationmonth <- human$creationmonth[whichtocopy]
  tempdf$doy <- human$doy[whichtocopy]
  tempdf$weeknum <- human$weeknum[whichtocopy]
  
  human <- bind_rows(human, tempdf)
...
if (!(mosqsummarystyle %in% c("stratifiedMIGR",
                            "stratifiedMII"))) {
...
if (!(mosqsummarystyle %in% c("stratifiedMIGR", "stratifiedMII"))) {
  ...
  yearfiller <- expand.grid(year = unique(fullcasemat$year),
                            stratum = unique(strata$strata))
  yearfiller$stratumyear <- paste(yearfiller$stratum,
                                  yearfiller$year,
                                  sep="-")
  yearfiller <- left_join(yearfiller,
                          randeffs[c("stratumyear", "MIRsummarystat")],
                          by="stratumyear")
  yearfiller$shouldimpute <- is.na(yearfiller$MIRsummarystat) & (yearfiller$year %in% humanbelievableyears)
...
yearfiller$MIRsummarystat[is.na(yearfiller$MIRsummarystat)] <- 0
...
if (!(mosqsummarystyle %in% c("stratifiedMIGR", "stratifiedMII"))) {
  ...
  fullcasemat <- left_join(fullcasemat,
                           yearfiller[c("stratumyear",
                                        "MIRsummarystat",
                                        "shouldimpute")],
                           by="stratumyear")
  
}
fullcasemat$MIRsummarystat[is.na(fullcasemat$MIRsummarystat)] <- 0
```
...
## Predictions of positive districts for the current week
...
# tempdf1 <- dplyr::summarise(group_by(fullcasemat, year),
#                             var1      =mean(var1[1,], na.rm=TRUE),
#                             var2      =mean(var2[2,], na.rm=TRUE),
#                             totalcases=sum(totalcases, na.rm=TRUE),
#                             totalpcws =sum(anycases, na.rm=TRUE),
#                             totalest  =sum(pred, na.rm=TRUE),
#                             mosquito  =mean(MIRsummarystat, na.rm=TRUE))
# thisplot1 <- ggplot(tempdf1) + geom_point(aes(x=mosquito, y=totalcases)) +
#   xlab("modeleded mosquito infection rate (relative)") +
#   ylab("total cases that year")
# ggsave(plot=thisplot1, filename="total vs mosquito.png", height=6, width=6, units="in")
# write.csv(tempdf1, file="total vs mosquito.csv")
```
...
###################################
forecast_date <- weekinquestionSun
forecast_year <- as.numeric(format(forecast_date, "%Y"))
forecast_week  <- as.numeric(format(forecast_date, "%U"))
outfile_name <- paste(params$state_code, forecast_year, forecast_week, sep="_")
outfile_name <- paste0(outfile_name, ".csv")

outdata <- data.frame(forecast_date,
                      forecast_year,
                      forecast_week, 
                      preds)

write.csv(outdata, outfile_name)
###################################
...
`r normalizePath(params$data_path, winslash = "/")`

```{r printparams, include=TRUE, echo=FALSE}
param_table <- data.frame(names(params), unlist(params))
names(param_table) <- c("Parameter", "Value")
kable(param_table[1:nrow(param_table)-1,], 
      caption = "Table of Rmd script parameters",
      row.names = FALSE)
```
